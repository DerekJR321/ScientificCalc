...
(window => {
    ...
    const secondKeySet = [].slice.call(calcSS3.querySelector('.calc-left').children, 12, 20);
    const hiddenCopy = calcSS3.querySelector('textarea');
    let pressedKey;
    ...
    class Calculator {
        constructor() { // for every '(' a new instance
			this.stack = [],
			this.num = 0,
			this.res = 0,
        ...
        calc(key, val) {
            ...
            if (key === '=' && !this.stack[0] && this.curr && this.buff[1]) { // repeating '='
                return `${this.buff[1] === 'yx' ? val ** this.buff[0] : this.buff[1] === 'x√y' ?
    val ** (1 / this.buff[0]) : [1] === 'EE' ? val * (10 ** this.buff[0]) :
    eval(`(${val})${this.buff[1]}(${this.buff[0]})`)}`;
            ...
            if (rank[key] <= rank[this.stack[this.num - 1][1]]) {
                this.stack[this.num - 1] = [
                    this.stack[this.num - 1][1] === 'yx' ? this.stack[this.num - 1][0] ** val :
                    this.stack[this.num - 1][1] === 'x√y' ? this.stack[this.num - 1][0] ** (1 / val) :
                    this.stack[this.num - 1][1] === 'EE' ? this.stack[this.num - 1][0] * (10 ** val) :
    ...
    // Collect all keys
    for (let k = 2; k--; ) {
		for (let l = calcSS3.children[k + 1], m = l.children, n = m.length; n--; ) {
    ...
    document.addEventListener('keypress', ({which}) => {
        let key = which;
        const holdKey = hold.textContent;

        const keyMatch = (',|.|-|–|/|÷|*|×|#|+/–|x|x!|E|EE|e|ex| |2nd|r|x√y|R|√|p|π|^|yx|\'|yx|"|yx|m|mr|v|mc|b|m+|n|m-|' +
            's|sin|c|cos|t|tan|S|sin-1|C|cos-1|T|tan-1|d|Deg|°|Deg|l|ln|L|log|\\|1/x|X|2x').split('|');

        const keyMatchHold = ('sin|sinh|cos|cosh|tan|tanh|m-|Rand|Deg|Rand|sin-1|sinh-1|cos-1|cosh-1|tan-1|tanh-1|' +
    ...
    document.addEventListener('keydown', e => {
        const str = resBuffer.replace(/\s/g, '');
    ...
    function render(val, inp) {
        ...
        // For common use: Get the values of pixels dynamically to stay free from design (...but outside this function)
			...
			switch (key) {
				...
				case 'x!':
					render(`${(function fak(n) {
                        return n < 0 || n > 170 ? NaN : n <= 1 ? 1 : n * fak(n - 1)})(Math.round(+dispVal))}`);
					break;
				case '√':
				...
				case 'log2':
					...
					break;
				case 'sinh':
					render(`${(Math.E ** dispVal - Math.E ** -dispVal) / 2}`);
					break;
				case 'sinh-1':
					render(`${Math.log(+dispVal + Math.sqrt(1 + dispVal ** 2))}`);
					break;
				case 'cosh':
					render(`${(Math.E ** dispVal + Math.E ** -dispVal) / 2}`);
					break;
